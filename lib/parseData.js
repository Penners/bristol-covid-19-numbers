...
const parseData = (response) => {
  if (response.E06000023 && response.metadata) {
    ...
    const yesterdayDate = todayDate.getTime() - 60 * 60 * 24 * 1000;
    ...
    response.E06000023.dailyTotalConfirmedCases.forEach((element) => {
      ...
      let temp = {
        dateFormat: format(new Date(key), "dd/MM/yyyy"),
        date: key,
    ...
    for (
      ...
      if (
        ...
        format(
          new Date(response.E06000023.dailyConfirmedCases[i].date),
          "yyyy-MM-dd"
        )
      ) {
      ...
      if (
        ...
        lastCaseDate = new Date(response.E06000023.dailyConfirmedCases[i].date);
      }
    }
